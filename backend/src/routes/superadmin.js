...
router.patch('/tenants/:id', async (req, res) => {
  try {
    const { active, name, primaryColor, saDeliveryDisabled, saRewardsDisabled, saPromoDisabled } = req.body;
    const tenant = await prisma.tenant.update({
      where: { id: parseInt(req.params.id) },
      data: { active, name, primaryColor, saDeliveryDisabled, saRewardsDisabled, saPromoDisabled }
    });
